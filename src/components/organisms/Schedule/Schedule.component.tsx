...
import { ScheduleProps, WeekDay } from './Schedule.interface';
...
export const Schedule = ({
  ...
  const unusedDays = allWeekDays.filter(
    (day) => days.find((d) => d.weekDay === day) === undefined
  );
  ...
  const addDay = () => {
    setDays(
      days.concat([{ weekDay: unusedDays[0], start: '12:00', end: '18:00' }])
    );
  };
        ...
        <HStack key={day.weekDay}>
          ...
          >
            {unusedDays.concat([day.weekDay]).map((selectDay) => (
              <option
                id={`schedule-${selectDay}`}
                value={selectDay}
                key={selectDay}
              >
                {mapDayToText[selectDay]}
              </option>
            ))}
          </Select>
